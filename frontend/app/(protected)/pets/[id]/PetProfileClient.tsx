...
const INP = "w-full px-2.5 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-800 outline-none focus:ring-2 focus:ring-indigo-200";
const STATUS_LABEL: Record<string, { t: string; c: string }> = {
    agendado: { t: "Agendado", c: "bg-sky-50 text-sky-700" },
    confirmado: { t: "Confirmado", c: "bg-teal-50 text-teal-700" },
    atendido: { t: "Atendido", c: "bg-emerald-50 text-emerald-700" },
    faltou: { t: "Faltou", c: "bg-rose-50 text-rose-700" },
    cancelado: { t: "Cancelado", c: "bg-slate-100 text-slate-600" },
};
...
export default function PetProfileClient({ pet: initialPet, appointments }: { pet: any; appointments: any[] }) {
    ...
    return (
        <div className="p-6 md:p-8 max-w-4xl mx-auto">
            <Link href="/pets" className="inline-flex items-center gap-1 text-sm text-slate-600 hover:text-slate-800 mb-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 mb-4">
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {tutor && (
                            <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-slate-600">
                                <span className="font-medium text-slate-700">{tutor.name}</span>
                    ...
                    {!editing && (
                        <button onClick={() => setEditing(true)} aria-label="Editar prontuário" className="text-slate-600 hover:text-indigo-600 hover:bg-slate-50 rounded-lg flex items-center gap-1 text-sm font-medium shrink-0 px-2.5 py-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                    <h2 className="text-base font-bold text-slate-800 mb-3">Dados</h2>
                    {!editing ? (
                        ...
                        <div className="space-y-2">
                            ...
                            <Field label="Observações"><textarea rows={2} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="w-full px-2.5 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-800 outline-none focus:ring-2 focus:ring-indigo-200 resize-none" /></Field>
                            <div className="flex gap-2 pt-1">
                                ...
                                <button onClick={() => setEditing(false)} aria-label="Cancelar edição" className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg"><X size={14} /></button>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                    <div className="flex items-center justify-between mb-3">
                        <h2 className="text-base font-bold text-slate-800 flex items-center gap-1.5"><Syringe size={15} className="text-indigo-500" /> Vacinas</h2>
                    ...
                    <div className="divide-y divide-slate-100">
                        {vaccines.length === 0 && !addingVac && <p className="text-xs text-slate-500 italic">Nenhuma vacina registrada.</p>}
                        {vaccines.map((v) => {
                            ...
                            return (
                                <div key={v.id} className="flex items-center gap-2 py-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-1.5"><span className="text-xs font-semibold text-slate-800">{v.vaccine_name}</span>{st && <span className={`text-[9px] px-1 py-0.5 rounded-full font-semibold ${st.cls}`}>{st.label}</span>}</div>
                                        <span className="text-[10px] text-slate-500">{v.applied_at && `Aplicada ${fmt(v.applied_at)}`}{v.next_due_at && ` · Próxima ${fmt(v.next_due_at)}`}</span>
                                    </div>
                                    <button onClick={() => handleDeleteVac(v.id)} aria-label={`Remover vacina ${v.vaccine_name}`} className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg p-2.5 shrink-0"><Trash2 size={14} /></button>
                                </div>
                            );
                        })}
                        {addingVac && (
                            <div className="pt-3 mt-1 space-y-2">
                                <div className="flex flex-wrap gap-1">
                                    {COMMON_VACCINES.map((n) => <button key={n} onClick={() => setVac({ ...vac, vaccineName: n })} className={`text-[10px] px-1.5 py-0.5 rounded border ${vac.vaccineName === n ? "bg-indigo-100 border-indigo-300 text-indigo-700" : "bg-slate-50 border-slate-200 text-slate-600"}`}>{n}</button>)}
                                </div>
                                <input placeholder="Vacina" value={vac.vaccineName} onChange={(e) => setVac({ ...vac, vaccineName: e.target.value })} aria-label="Nome da vacina" className="w-full px-2 py-1.5 border border-slate-200 rounded text-xs text-slate-800 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-indigo-200" />
                                <div className="grid grid-cols-2 gap-2">
                                    <label className="text-[10px] text-slate-600">Aplicada<input type="date" value={vac.appliedAt} onChange={(e) => setVac({ ...vac, appliedAt: e.target.value })} className="w-full px-1.5 py-1.5 border border-slate-200 rounded text-xs text-slate-800 outline-none focus:ring-2 focus:ring-indigo-200" /></label>
                                    <label className="text-[10px] text-slate-600">Próxima<input type="date" value={vac.nextDueAt} onChange={(e) => setVac({ ...vac, nextDueAt: e.target.value })} className="w-full px-1.5 py-1.5 border border-slate-200 rounded text-xs text-slate-800 outline-none focus:ring-2 focus:ring-indigo-200" /></label>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={handleAddVac} disabled={savingVac} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold py-1.5 rounded flex items-center justify-center gap-1 disabled:opacity-60">{savingVac ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />} Salvar</button>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 mt-4">
                <h2 className="text-base font-bold text-slate-800 flex items-center gap-1.5 mb-3"><Calendar size={15} className="text-indigo-500" /> Histórico de atendimentos</h2>
                {appointments.length === 0 ? (
                    <p className="text-xs text-slate-500 italic">Nenhum atendimento registrado ainda.</p>
                ) : (
                    <div className="divide-y divide-slate-100">
                        {appointments.map((a) => {
                            ...
                            return (
                                <div key={a.id} className="flex items-center gap-3 py-2.5">
                                    <div className="text-xs text-slate-500 w-20 shrink-0 tabular-nums">{fmt(a.starts_at)} {a.starts_at?.slice(11, 16)}</div>
                                    <div className="flex-1 min-w-0">
                                        <span className="text-sm font-medium text-slate-800">{a.service_name || "Atendimento"}</span>
                                        {a.professional?.full_name && <span className="text-[11px] text-slate-500 flex items-center gap-1"><Stethoscope size={11} /> {a.professional.full_name}</span>}
                                    </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
    return <label className="block text-[11px] font-semibold text-slate-600 col-span-1">{label}{children}</label>;
}
