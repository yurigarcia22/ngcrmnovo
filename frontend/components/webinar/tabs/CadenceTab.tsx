...
const SUGGESTED_CADENCE = [
  ...
  { day: -3, name: "Reforço", template: "Oi {empresa}, lembrete que o webinar {tema} acontece em 3 dias. Confirma sua presença?" },
  { day: -1, name: "Véspera", template: "{empresa}, o webinar é amanhã às {hora}. Link: {meet_link}" },
  { day: 0, name: "1h antes", template: "{empresa}, em 1 hora começamos o webinar. Salva o link: {meet_link}" },
  { day: 0, name: "10 min antes", template: "{empresa}, está começando agora. Entra: {meet_link}" },
  { day: 1, name: "Após evento (presente)", template: "Que bom te ver no webinar! Quer agendar uma call de diagnóstico? {cal_link}" },
  { day: 1, name: "Após evento (ausente)", template: "Senti tua falta no webinar de hoje. Quer marcar um diagnóstico? {cal_link}" },
];
    ...
    <div className="space-y-6 max-w-4xl">
      <Card className="p-6">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-800">Cadência da campanha</h2>
            <p className="text-xs text-slate-500 mt-1">
              Configure as mensagens que serão disparadas em cada momento da cadência.
              Use variáveis: <code className="text-[11px] bg-slate-100 px-1 py-0.5 rounded">{"{empresa}"}</code>{" "}
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-bold text-slate-800">Sugestão de cadência padrão</h2>
        ...
        <div className="mt-4 pt-4 border-t border-slate-100">
          <p className="text-xs text-slate-400 italic">
            Editor de cadência interativo disponível na Fase 4. Por enquanto, ajusta os
